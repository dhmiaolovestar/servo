...
def _control(x , y):
    ...
    if x < 160 :                       #2.3最左;0.7最右;1.6中间
        angle_down=angle_down+0.07*math.fabs(x-160)#左移
    elif (160 < x < 320) :
        angle_down=angle_down-0.07*math.fabs(x-160)#右移
    if y < 60 :                      #1.5正向上;0.4最下;0.7正向前
        angle_up=angle_up-0.1*math.fabs(y-60)    #下移
    elif (60 < y < 120) :
        angle_up=angle_up+0.1*math.fabs(y-60)    #上移
...
pid_up=PositionalPID( 0.5 , 0.1 , 0.0 )
pid_down=PositionalPID( 0.5 , 0.1 , 0.0 )
...
while True:
    ...
    if blobs:   #如果找到了目标颜色
        for b in blobs:#迭代找到的目标颜色区域
            ...
            #pid控制
            pid_up.SetStepSignal( err_normalization( 240 , b[5] ) )
            pid_up.control( 1 , pid_up.PIDOutput )

            pid_down.SetStepSignal( err_normalization( 360 , b[6] ) )
